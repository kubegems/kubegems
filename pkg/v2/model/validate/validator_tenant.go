...
func TenantStructLevelValidation(sl validator.StructLevel) {
	/*
		...
		if tenant.ID == 0 && len(tenant.TenantName) > 0 {
			var count int64
			if models.GetDB().Find(&tmp, "tenant_name = ?", tenant.TenantName).Count(&count); count != 0 {
		...
		if tenant.ID != 0 {
			var count int64
			if models.GetDB().Find(&tmp, "tenant_name = ? and id <> ?", tenant.TenantName, tenant.ID).Count(&count); count != 0 {
				sl.ReportError(tenant.TenantName, "租户名字", "TenantName", "dbuniq", "租户")
			}
		}
	*/
...
func TenantUserRelStructLevelValidation(sl validator.StructLevel) {
	/*
		...
		if rel.ID == 0 {
			var count int64
			if models.GetDB().Find(&tmp, "user_id = ? and tenant_id = ?", rel.UserID, rel.TenantID).Count(&count); count != 0 {
