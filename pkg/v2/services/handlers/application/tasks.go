...
// @Security     JWT
func (h *TaskHandler) List(req *restful.Request, resp *restful.Response) {
	h.NamedRefFunc(req, resp, nil, func(ctx context.Context, ref PathRef) (interface{}, error) {
		...
		sse.Encode(resp, sse.Event{Event: "data", Data: tasks})
		resp.Flush()
...
func (h *TaskHandler) BatchList(req *restful.Request, resp *restful.Response) {
	names := []string{}
	if val := req.QueryParameter("names"); val != "" {
		names = strings.Split(req.QueryParameter("names"), ",")
	}
