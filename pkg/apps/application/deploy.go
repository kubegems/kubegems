...
func MustNewApplicationDeployHandler(provider git.Provider, argocli *argo.Client, commonbase base.BaseHandler) *ApplicationHandler {
	...
	tp := &TaskProcessor{Workflowcli: workflow.NewDefaultRemoteClient()}
	h := &ApplicationHandler{
		...
		Task:                 NewTaskHandler(base, tp),
		ApplicationProcessor: NewApplicationProcessor(database, provider, argocli, tp, agents),
	}
	return h
}
