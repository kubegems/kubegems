...
func MigrateDatabaseAndInitData(opts *database.Options, initData bool) error {
	...
	if err := migrateModels(db.DB().Debug()); err != nil {
		return err
	}

	if initData {
		if err := initBaseData(db.DB().Debug()); err != nil {
			return err
		}
	}
	return nil
...
func initBaseData(db *gorm.DB) error {
	active := true
	sysadmin := SystemRole{ID: 1, Name: "系统管理员", Code: "sysadmin"}
	normal := SystemRole{ID: 2, Name: "普通用户", Code: "normal"}
	admin := User{
		ID:           1,
		Username:     "admin",
		IsActive:     &active,
