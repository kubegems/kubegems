...
func (h *ApplicationHandler) StrategyDeploymentControl(req *restful.Request, resp *restful.Response) {
	...
	ctrlfunc := func(ctx context.Context, store GitStore, cli agents.Client, namespace string, ref PathRef) (interface{}, error) {
		log.FromContextOrDiscard(ctx).Info("strategy deployment control", "command", control.Command, "args", control.Args)
		...
		case "promote":
			// 兼容两种类型
			full := false
			switch v := control.Args["full"].(type) {
			case bool:
				full = v
			case string:
		...
		case "terminate":
			// TODO:
		case "undo":
			revison, _ := control.Args["revision"].(string)
			if revison == "" {
				return nil, fmt.Errorf("undo must have revision specified")
			}
			if issync, _ := strconv.ParseBool(req.QueryParameter("sync")); issync {
	...
	h.LocalAndRemoteCliFunc(req, resp, control, ctrlfunc, "rollout ctrl")
}
